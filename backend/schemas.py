...
class AgentExecutionEvent(BaseModel):
    ...
    status: Literal["running", "complete", "error"] = Field(default="complete", description="Status of the agent execution")
    input_summary: str = Field(description="Human-readable summary of what the agent received")
    input_data: dict[str, Any] = Field(default_factory=dict, description="Detailed JSON input data the agent received")
    output_data: dict[str, Any] = Field(default_factory=dict, description="The exact JSON output produced by the agent")
    processing_logs: list[str] = Field(default_factory=list, description="Step-by-step processing logs for transparency")
